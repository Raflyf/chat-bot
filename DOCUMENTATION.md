...
- **Cron reminder** (`vercel.json`): endpoint `/api/cron/reminders` siap (auth Bearer CRON_SECRET, fail-closed). **CATATAN PLAN: Vercel Hobby hanya mengizinkan cron harian** — untuk reminder per-menit pasang scheduler eksternal (cron-job.org / GitHub Actions / UptimeRobot) yang memanggil `GET https://<domain>/api/cron/reminders` dengan header `Authorization: Bearer <CRON_SECRET>` setiap 1-5 menit. Worker lokal menghormati `platform` (reminder WhatsApp tidak lagi salah kirim ke Telegram).
...
- **Sinkronisasi Backend Monitoring (`api/stats.ts`)**:
  ...
  - Definisi kuota valid per key & pool:
    ...
    - **OpenCode Zen API (Tier 3):** 4 Keys • ~1.000 RPD/key • Contributor Free (Bebas token harian) • Context 1.048.576 (1M) • Teks & Penalaran Empatik (REST API teks murni; vision ditangani Tier Vision).
